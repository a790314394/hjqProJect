			...
			<td class="rightimg">
				...
				<div id="right">
					...
					<form id="form1" name="form1" method="post"
						action="insertTaskInfo.do">
						<p>
							任务名称：<label> <input name="taskName" type="text" size="50" />
							</label>
						</p>
						<p>
							任务描述： <label> <textarea name="taskDesc"></textarea> </label>
						</p>
						<p>
							开始时间： <label> <input name="beginDate" type="text"
								size="16" /> </label> <span class="marginleft1">结束时间： <label>
									<input name="endDate" type="text" size="16" /> </label> </span>
						</p>
						<p>
							实施人员： <label> <select name="taskId">
									<c:forEach items="${sessionScope.EmpMap }" var="emp">
										<option value="${emp.key}">${emp.value }</option>
									</c:forEach>
							</select> </label> <span class="marginleft">任务状态： 待实施 </span>
						</p>
						<p>
							<label> <input name="Submit" type="reset" class="menu2"
								value="重置" /> </label> &nbsp; <label> <input name="Submit2"
								type="submit" class="menu1" value="提交" /> </label>
						</p>
					</form>
				</div>
			</td>
